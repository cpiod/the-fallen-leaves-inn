...
func start_dialog(s: String, label: String = ""):
	Dialogic.timeline_ended.connect(_on_timeline_ended)
	can_play = false
	Dialogic.start(s, label)
...
func _on_texture_button_pressed() -> void:
	...
	print(edible_correct, other_correct)
	if !edible_correct and !other_correct:
		start_dialog("fantome", "0-correct")
	elif edible_correct and other_correct:
		start_dialog("fantome", "2-correct")
	elif edible_correct:
		start_dialog("fantome", "wrong-other")
	elif other_correct:
		start_dialog("fantome", "wrong-edible")
	tries += 1
